...
std::map<std::string, int>& lzw::build_dictionary() {
  ...
  for (int x = 0; x < dictionary_size; ++x)
...
Iter compress(const std::vector<char> v, Iter result) {
...
std::string decompress(Iter begin, Iter end) {
  ...
  for (int i = 0; i < 256; ++i)
    dictionary.insert[std::pair<int, std::string>(i, std::string(1, i))];
...
std::string lzw::int_to_binary(int c, int cl) {
  ...
  int zeros = cl-p.size();
  if (zeros < 0) {
     std::cout << "\nWarning: Overflow. code " << c << " is too big to be coded by " << cl << " bits!\n";
...
void binaryIODemo(std::vector<int> compressed) {
   ...
   for (std::vector<int>::iterator it = compressed.begin(); it != compressed.end(); ++it) {
      ...
      std::cout << "c=" << *it << " : binary string=" << p << "; back to code=" << binary_to_int(p) << "\n";
      bcode += p;
   }
   ...
   if (bcode.size() % 8) // Make sure the length of the binary string is a multiple of 8
